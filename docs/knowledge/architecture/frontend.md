...
description: Details the modular client-side SPA architecture, state management, View Transitions-based routing, and BUILD_ID cache-busted API fetches.
...
Crawler-facing post URLs are **not** SPA routes: they are pre-rendered [Static Post Pages](../glossary.md) under `posts/`. The SPA remains the interactive reading experience (hash routing, filters, themes).

## Core Architecture Components

The application state and presentation flow are driven by modules in `/assets/js/`:
...
- **Features**:
  - Fetches the static index `api/index.json?v=${BUILD_ID}` on startup (see [BUILD_ID](../glossary.md)).
  ...
  - Relies on normal HTTP caching (no `cache: "no-store"`); `BUILD_ID` busts caches after content publishes.
...
- **Render Targets**:
  ...
  - Post detail view (lazy-loads `api/posts/{slug}.json?v=${BUILD_ID}`; falls back to static HTML if needed).
...
## Relevant Files
- [main.js](../../../assets/js/main.js) (entry point)
- [store.js](../../../assets/js/store.js)
- [renderer.js](../../../assets/js/renderer.js)
- [router.js](../../../assets/js/router.js)
- [build-id.js](../../../assets/js/build-id.js) (auto-generated by `build-static-api.js`)
